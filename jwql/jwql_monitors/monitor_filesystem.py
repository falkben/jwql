...
def monitor_filesystem():
    ...
    with open(statsfile, "a+") as f:
        f.write(
            "{0} {1:15d} {2:15d} {3:15d} {4:15d} {5}\n".format(
                now, results_dict["file_count"], total, available, used, percent_used
            )
        )
...
def plot_system_stats(stats_file, filebytype, sizebytype):
    ...
    date, f_count, sysize, frsize, used, percent = np.loadtxt(
        stats_file, dtype=str, unpack=True
    )
    ...
    logging.info(
        "Read in file statistics from {}, {}, {}".format(
    ...
    systemsize = sysize.astype(float) / (1024.0 ** 3)
    freesize = frsize.astype(float) / (1024.0 ** 3)
    usedsize = used.astype(float) / (1024.0 ** 3)
    ...
    fits_size = fits_sz.astype(float) / (1024.0 ** 3)
    uncal_size = uncal_sz.astype(float) / (1024.0 ** 3)
    cal_size = cal_sz.astype(float) / (1024.0 ** 3)
    rate_size = rate_sz.astype(float) / (1024.0 ** 3)
    rateints_size = rateints_sz.astype(float) / (1024.0 ** 3)
    i2d_size = i2d_sz.astype(float) / (1024.0 ** 3)
    nircam_size = nrc_sz.astype(float) / (1024.0 ** 3)
    nirspec_size = nrs_sz.astype(float) / (1024.0 ** 3)
    niriss_size = nis_sz.astype(float) / (1024.0 ** 3)
    miri_size = mir_sz.astype(float) / (1024.0 ** 3)
    fgs_size = fgs_sz.astype(float) / (1024.0 ** 3)
    ...
    for plot, name in zip(plots, plot_names):
        ...
        logging.info(
            "Saved components files: {}_component.html and {}_component.js".format(
                name, name
            )
        )

...
def get_acknowledgements():
    ...
    acknowledgements = [
        item.strip().replace("- ", "").split(" [@")[0].strip()
...
def get_dashboard_components():
    ...
    with open(
        os.path.join(output_dir, "monitor_cron_jobs", "cron_status_table.html")
    ) as f:
...
def get_edb_components(request):
    ...
    if request.method == "POST":
        ...
        if "mnemonic_name_search" in request.POST.keys():
            ...
            mnemonic_query_form = MnemonicQueryForm(
                request.POST, prefix="mnemonic_query"
            )
            ...
            if mnemonic_query_form.is_valid():
                ...
                start_time = Time(
                    mnemonic_query_form["start_time"].value(), format="iso"
                )
...
def get_filenames_by_instrument(instrument):
    ...
    filepaths = [
        f for f in glob.glob(search_filepath) if instrument_match[instrument] in f
    ]
...
def get_filenames_by_proposal(proposal):
    ...
    filenames = sorted(
        glob.glob(os.path.join(FILESYSTEM_DIR, "jw{}".format(proposal), "*"))
    )
...
def get_filenames_by_rootname(rootname):
    ...
    filenames = sorted(
        glob.glob(
            os.path.join(
                FILESYSTEM_DIR, "jw{}".format(proposal), "{}*".format(rootname)
            )
        )
    )
    filenames = [os.path.basename(filename) for filename in filenames]
...
def get_image_info(file_root, rewrite):
    ...
    for file in image_info["all_files"]:
        ...
        search_jpgs = os.path.join(
            preview_dir, dirname, file_root + "_{}_integ*.jpg".format(suffix)
        )
...
def get_preview_images_by_proposal(proposal):
    ...
    preview_images = glob.glob(
        os.path.join(PREVIEW_IMAGE_FILESYSTEM, "jw{}".format(proposal), "*")
    )
    preview_images = [
        os.path.basename(preview_image) for preview_image in preview_images
    ]
...
def get_thumbnails_by_proposal(proposal):
    ...
    thumbnails = glob.glob(
        os.path.join(THUMBNAIL_FILESYSTEM, "jw{}".format(proposal), "*")
    )
...
def get_thumbnails_by_rootname(rootname):
    ...
    thumbnails = sorted(
        glob.glob(
            os.path.join(
                THUMBNAIL_FILESYSTEM, "jw{}".format(proposal), "{}*".format(rootname)
            )
        )
    )
...
def split_files(file_list, page_type):
    ...
    if page_type == "unlooked":
        print(
            'ONLY RETURNING {} "UNLOOKED" FILES OF {} ORIGINAL FILES'.format(
                len([m for m in mask_unlooked if m]), len(file_list)
            )
        )
        return [f for i, f in enumerate(file_list) if mask_unlooked[i]]
    elif page_type == "archive":
        print(
            'ONLY RETURNING {} "ARCHIVED" FILES OF {} ORIGINAL FILES'.format(
                len([m for m in mask_unlooked if not m]), len(file_list)
            )
        )
